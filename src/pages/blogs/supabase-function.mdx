...
```typescript
    ...
    export async function incrementCountForUser(userID: number, count_field: string) {
        const field_to_increment = `count_${count_field}`  // count_a or count_b
        const { data, error } = await supabaseBrowser.rpc('increment', { user_id_to_inc: userID, field_name: field_to_increment })
        if (error) {
            console.log(error);
        }
    }
